...
public class FhirServerConfigCommon {
  ...
  private static final org.slf4j.Logger ourLog =
      org.slf4j.LoggerFactory.getLogger(FhirServerConfigCommon.class);
  ...
  public FhirServerConfigCommon(AppProperties appProperties) {
    ...
    ourLog.info(
        "Server configured to "
            + (appProperties.getExpunge_enabled() ? "enable" : "disable")
            + " expunges");
    ourLog.info(
        "Server configured to "
            + (appProperties.getAllow_placeholder_references() ? "allow" : "deny")
  ...
  public DaoConfig daoConfig(AppProperties appProperties) {
    ...
    retVal.setEnforceReferentialIntegrityOnWrite(
  ...
  public ModelConfig modelConfig(AppProperties appProperties) {
    ...
    if (appProperties.getSubscription() != null
        && appProperties.getSubscription().getResthook_enabled() != null) {
    ...
    modelConfig.setNormalizedQuantitySearchLevel(
        appProperties.getNormalized_quantity_search_level());
    return modelConfig;
  }

  /**
   * The following bean configures the database connection. The 'url' property value of
   * "jdbc:derby:directory:jpaserver_derby_files;create=true" indicates that the server should save
   * resources in a directory called "jpaserver_derby_files".
   *
   * <p>A URL to a remote database could also be placed here, along with login credentials and other
