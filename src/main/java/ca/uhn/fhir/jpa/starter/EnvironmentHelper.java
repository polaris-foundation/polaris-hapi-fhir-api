...
public class EnvironmentHelper {
  ...
  public static Properties getHibernateProperties(ConfigurableEnvironment environment) {
    ...
    properties.putIfAbsent(
        AvailableSettings.SCANNER, "org.hibernate.boot.archive.scan.internal.DisabledScanner");
    properties.putIfAbsent(
    ...
    // from the entityManagerFactory bean from xBaseConfig
    ...
    Map<String, Object> hapiJpaPropertyMap =
        new HapiFhirLocalContainerEntityManagerFactoryBean().getJpaPropertyMap();
    ...
    if (Boolean.parseBoolean(String.valueOf(properties.get(HibernateOrmMapperSettings.ENABLED)))) {
      ...
      if (properties
        ...
        ElasticsearchHibernatePropertiesBuilder builder =
        ...
        SchemaManagementStrategyName indexSchemaManagementStrategy =
        ...
        if (refreshAfterWrite == null || !refreshAfterWrite) {
          ...
          builder.setDebugIndexSyncStrategy(
              AutomaticIndexingSynchronizationStrategyNames.READ_SYNC);
        }
        builder.setDebugPrettyPrintJsonLog(
            requireNonNullElse(
                environment.getProperty("elasticsearch.debug.pretty_print_json_log", Boolean.class),
                false));
        builder.apply(properties);

      } else {
        throw new UnsupportedOperationException(
            "Unsupported Hibernate Search backend: "
