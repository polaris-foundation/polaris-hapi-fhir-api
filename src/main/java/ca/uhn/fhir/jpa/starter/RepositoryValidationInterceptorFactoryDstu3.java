...
/**
 * This class can be customized to enable the {@link RepositoryValidatingInterceptor} on this
 * server.
 *
...
@ConditionalOnProperty(
    prefix = "hapi.fhir",
    name = "enable_repository_validating_interceptor",
    havingValue = "true")
@Configuration
@Conditional(OnDSTU3Condition.class)
public class RepositoryValidationInterceptorFactoryDstu3
    implements IRepositoryValidationInterceptorFactory {
  ...
  public RepositoryValidatingInterceptor buildUsingStoredStructureDefinitions() {
    ...
    IBundleProvider results =
        structureDefinitionResourceProvider.search(
            new SearchParameterMap().add(StructureDefinition.SP_KIND, new TokenParam("resource")));
    Map<String, List<StructureDefinition>> structureDefintions =
        results.getResources(0, results.size()).stream()
    ...
    structureDefintions
        .entrySet()
        .forEach(
            structureDefinitionListEntry -> {
              String[] urls =
                  structureDefinitionListEntry.getValue().stream()
                      .map(StructureDefinition::getUrl)
                      .toArray(String[]::new);
              repositoryValidatingRuleBuilder
                  .forResourcesOfType(structureDefinitionListEntry.getKey())
