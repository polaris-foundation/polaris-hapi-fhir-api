...
/**
 * TODO: Move this to package "ca.uhn.fhir.jpa.starter" in HAPI FHIR 5.2.0+. The lousy component
 * scan in 5.1.0 picks this up even if MDM is disabled currently.
 */
@Configuration
...
public class MdmConfig {
  ...
  MdmRuleValidator mdmRuleValidator(
      FhirContext theFhirContext, ISearchParamRetriever theSearchParamRetriever) {
    return new MdmRuleValidator(theFhirContext, theSearchParamRetriever);
  }

  @Bean
  IMdmSettings mdmSettings(
      @Autowired MdmRuleValidator theMdmRuleValidator, AppProperties appProperties)
      throws IOException {
